<template>
  <div class="plot-details-panel factory-panel">
    <!-- 装饰线 -->
    <img class="panel-decoration-top" src="/images/decoration-1.jpg" />

    <!-- 地块标题信息 -->
    <div class="plot-title-section">
      <h2 class="left-plot-name">{{ plotName }}</h2>
      <img class="region-label" src="/images/region-label.jpg" />
      <span class="region-name">{{ regionName }}</span>
    </div>

    <img class="section-divider" src="/images/decoration-2.png" />

    <!-- 所有人信息 -->
    <FarmerProfileCard
      :farmer-name="strategy.getOwnerInfo().name"
      :farmer-age="strategy.getOwnerInfo().factoryCount"
      :avatar-url="strategy.getOwnerInfo().avatar"
      :background-image="'/images/farmer-profile.png'"
      :status-tags="strategy.getStatusTags()"
      :rating="strategy.getOwnerInfo().rating"
      label-prefix="所有人："
      age-label="名下工厂数："
      age-suffix="个"
    />

    <!-- 工厂统计数据 -->
    <PlotStatisticsGrid
      :items="strategy.getStatistics()"
      :background-image="'/images/stat-item.png'"
    />

    <!-- 鲜果收购价信息 -->
    <div class="price-info" :style="{ backgroundImage: 'url(/images/price-info.png)' }">
      <div class="price-display">
        <span class="price-label">鲜果收购价：</span>
        <img class="down-arrow" src="/images/down-arrow.png" />
        <span class="price-value">{{ priceInfo.freshFruitPrice }}</span>
        <span class="price-unit">&nbsp;&nbsp;元/斤</span>
      </div>
    </div>

    <!-- 加工价格信息 -->
    <div class="price-info" :style="{ backgroundImage: 'url(/images/price-info.png)' }">
      <div class="price-display">
        <span class="price-label">加工价格：</span>
        <img class="down-arrow" src="/images/down-arrow.png" />
        <span class="price-value">{{ priceInfo.processingPrice }}</span>
        <span class="price-unit">&nbsp;&nbsp;元/吨</span>
      </div>
    </div>

    <!-- 施工计划（日历形式） -->
    <div class="health-section">
      <div class="health-header">
        <span class="health-title">施工计划</span>
      </div>
      <img class="third-divider" src="/images/decoration-2.png" />

      <!-- 施工计划日历 -->
      <ConstructionCalendar
        v-if="calendarData"
        :calendar-data="calendarData"
      />
    </div>
  </div>
</template>

<script>
import FarmerProfileCard from '@/components/PlotDetail/FarmerProfileCard.vue';
import PlotStatisticsGrid from '@/components/PlotDetail/PlotStatisticsGrid.vue';
import ConstructionCalendar from '@/components/PlotDetail/ConstructionCalendar.vue';

/**
 * 工厂左侧面板
 *
 * 显示工厂详情信息
 * @component FactoryLeftPanel
 */
export default {
    name: 'FactoryLeftPanel',
    components: {
        FarmerProfileCard,
        PlotStatisticsGrid,
        ConstructionCalendar
    },
    props: {
        // 策略实例
        strategy: {
            type: Object,
            required: true
        },
        // 地块名称
        plotName: {
            type: String,
            required: true
        },
        // 区域名称
        regionName: {
            type: String,
            required: true
        },
        // 价格信息
        priceInfo: {
            type: Object,
            required: true
        },
        // 日历数据
        calendarData: {
            type: Object,
            default: null
        }
    }
};
</script>

<style lang="less" scoped>
/* 基础容器样式 - 必须保留以确保布局正确 */
.plot-details-panel {
    position: relative;
    display: flex;
    overflow-x: hidden;
    overflow-y: auto;
    flex-direction: column;
    box-sizing: border-box;
    width: 375px;
    min-height: 100%;
    margin: 0 0 0 5px;
    padding: 0 23px 35px;
    background-repeat: no-repeat;
    background-position: center;
    background-size: 100% 100%;
    gap: 10px;
}

/* 工厂面板特定背景 */
.factory-panel {
    background-color: #0d2a28;
    background-image: linear-gradient(135deg, #0d2a28f2 0%, #041f1df2 100%);
}

.panel-decoration-top {
    width: 100%;
    height: auto;
    margin-bottom: 35px;
    object-fit: fill;
}

.plot-title-section {
    display: flex;
    align-items: center;
    justify-content: space-between;
}

.left-plot-name {
    flex: 1;
    margin: 0;
    font-family: SourceHanSansCN-Medium, sans-serif;
    font-size: 17px;
    font-weight: 500;
    color: #c69c6d;
}

.region-label {
    width: 13px;
    height: 18px;
    margin-right: 9px;
}

.region-name {
    font-family: SourceHanSansCN-Medium, sans-serif;
    font-size: 16px;
    font-weight: 500;
    color: #39b44a;
}

.section-divider {
    width: 160px;
    height: 3px;
    margin: 6px 0 0;
}

/* 价格信息 */
.price-info {
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: center;
    width: 315px;
    margin-top: 22px;
    padding: 15px 16px;
    background-size: 100% 100%;
}

.price-display {
    display: flex;
    flex-direction: row;
    align-items: center;
    justify-content: center;
    width: 100%;
    text-align: center;
    gap: 8px;
}

.price-label {
    font-family: SourceHanSansCN-Medium, sans-serif;
    font-size: 18px;
    font-weight: 500;
    color: #c69c6d;
}

.down-arrow {
    width: 11px;
    height: 20px;
    margin-right: 13px;
}

.price-value {
    font-family: BebasNeueRegular, sans-serif;
    font-size: 52px;
    line-height: 1;
    color: #c69c6d;
}

.price-unit {
    font-family: SourceHanSansCN-Medium, sans-serif;
    font-size: 18px;
    color: #c69c6d;
}

/* 健康指标/施工计划区域 */
.health-section {
    margin-top: auto;
    padding-top: 20px;
}

.health-header {
    display: flex;
    align-items: center;
    justify-content: space-between;
}

.health-title {
    font-family: SourceHanSansCN-Medium, sans-serif;
    font-size: 16px;
    font-weight: 500;
    color: #c69c6d;
}

.third-divider {
    display: block;
    width: 103px;
    height: 3px;
    margin-top: 5px;
    object-fit: contain;
}
</style>
